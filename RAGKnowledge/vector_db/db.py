...
from .qdrant_db import QdrantVectorDB, QdrantTextVectorDB, QdrantImageVectorDB
...
class VectorDB:
    ...
    def __init__(self, store_type: Optional[str] = None):
        ...
        logger.debug(f"向量数据库初始化完成，存储类型为: {self.db_type}")
    ...
    def base_store(self):
        ...
        if self._base_store is None:
            if self.db_type == "Qdrant":
                self._base_store = QdrantVectorDB(self.config)
            else:
                raise ValueError(f"不支持的向量数据库类型: {self.db_type}")

            logger.info(f"创建基础向量数据库实例: {self.db_type}")
    ...
    def text_store(self):
        ...
        if self._text_store is None:
            logger.info("创建文本向量存储实例")
            self._text_store = QdrantTextVectorDB(
                self.base_store, 
                self.config.text_collection, 
                self.config.text_dimension
            )
            logger.debug("创建文本向量存储实例完成")
    ...
    def image_store(self):
        ...
        if self._image_store is None:
            self._image_store = QdrantImageVectorDB(
                vector_db=self.base_store,
                collection_name=self.config.image_collection,
                embedding_size=self.config.image_dimension,
                index_fields=["kb_id", "doc_id", "image_id"]
            )
            logger.debug("创建图像向量存储实例完成")
    ...
    def page_store(self):
        ...
        if self._page_store is None:
            self._page_store = QdrantImageVectorDB(
                vector_db=self.base_store,
                collection_name=self.config.page_collection,
                embedding_size=self.config.page_dimension,
                index_fields=["kb_id", "page_id", "doc_id", "chunk_id"]
            )
            logger.debug("创建页面向量存储实例完成")
    ...
    def create_all_collections(self):
        """创建所有向量集合"""
        try:
            if self.text_store.create_collection():
                logger.info("文本向量集合创建成功")
            else:
                logger.error("文本向量集合创建失败")
        except Exception as e:
            logger.error(f"文本向量集合创建失败: {e}")

        try:
            if self.image_store.create_collection():
                logger.info("图像向量集合创建成功")
            else:
                logger.error("图像向量集合创建失败")
        except Exception as e:
            logger.error(f"图像向量集合创建失败: {e}")

        try:
            if self.page_store.create_collection():
                logger.info("页面向量集合创建成功")
            else:
                logger.error("页面向量集合创建失败")
        except Exception as e:
